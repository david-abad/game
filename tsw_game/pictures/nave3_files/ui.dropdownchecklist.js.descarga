...
**********/	
        ...
        _appendItems: function() {
            ...
            if(isNaN(divWidth))
            	divWidth = dropContainerDiv.outerWidth(true);

            var divHeight = dropContainerDiv.outerHeight();
            if(isNaN(divHeight))
            	divHeight = dropContainerDiv.outerHeight(true);
...
****/	
             		...
             		if ((config.positionHow == null) || (config.positionHow == 'absolute')) {
             			...
             			var topPx = instance.controlWrapper.position().top + instance.controlWrapper.outerHeight();
             			if(isNaN(topPx))
             				topPx = instance.controlWrapper.position().top + instance.controlWrapper.outerHeight(true);
        ...
        _setSize: function(dropCalculatedSize) {
            ...
            if ( controlIcon != null ) {
            	...
            	var coW  = controlIcon.outerWidth();
            	if(isNaN(coW))
            		coW  = controlIcon.outerWidth(true);
            	controlWidth -= (coW + 4);
            	controlText.css( { width: controlWidth + "px" } );
            }
            // Account for padding, borders, etc
            controlWidth = controlWrapper.outerWidth();
            if(isNaN(controlWidth))
            	controlWidth = controlWrapper.outerWidth(true);
